...
var iterExpit = require( './../lib' );


// VARIABLES //

var rand = randu( 0.0, 1.0 );
...
tape( 'main export is a function', function test( t ) {
	...
	t.equal( typeof iterExpit, 'function', 'main export is a function' );
	t.end();
});
...
tape( 'the function returns an iterator protocol-compliant object (value+done)', function test( t ) {
	...
	it = iterExpit( createIterator( values ) );
	t.equal( it.next.length, 0, 'has zero arity' );
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the standard logistic function for each iterated value', function test( t ) {
	var expected;
	var rand;
	var it;
	var N;
...
tape( 'the function returns an iterator protocol-compliant object which evaluates the standard logistic function for each iterated value (value+done)', function test( t ) {
	var expected;
	var actual;
	var values;
	var it;
	...
	expected = [
		{
			'value': expit( values[ 0 ] ),
			'done': false
		},
		{
			'value': expit( values[ 1 ] ),
			'done': false
		},
		{
			'value': expit( values[ 2 ] ),
			'done': false
		},
		{
			'value': expit( values[ 3 ] ),
			'done': true
		}
	];
